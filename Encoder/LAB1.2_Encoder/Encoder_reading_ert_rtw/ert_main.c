...
int main(int argc, char **argv)
{
  ...
  MX_TIM4_Init();
  MX_TIM1_Init();
  rtmSetErrorStatus(Encoder_reading_M, 0);
  ...
  Encoder_reading_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(Encoder_reading_M->extModeInfo, &rtmGetTFinal
    (Encoder_reading_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Encoder_reading_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Encoder_reading_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
